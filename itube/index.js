...
app.get(`/video`, async function(req, res) {
    ...
    let array = await Video.find({author: data.author, _id: {$ne: data.id}}).limit(3);
    res.render(`video`, {
        video: data,
        array: array
    });
...
app.get(`/blogger`, async function(req, res) {
    ...
    let array = await Video.find({author: username}).sort({likes: 1});
    res.render(`blogger`, {blogger: data, array: array});
});
